...
public:
  ...
  void setup_with_valid_lattice_model(double density) {
    ...
    m_pdf_field_id = lbm::addPdfFieldToStorage(
        m_blockforest->get_blocks(), "pdf field", *(m_lattice_model.get()),
    ...
    m_time_loop = std::make_shared<timeloop::SweepTimeloop>(
        m_blockforest->get_blocks()->getBlockStorage(), 1);
    ...
    m_velocity_adaptor_id = field::addFieldAdaptor<VelocityAdaptor>(
        m_blockforest->get_blocks(), m_pdf_field_id, "velocity adaptor");
  ...
  void on_lattice_model_change() {
    for (auto b = m_blockforest->get_blocks()->begin();
      ...
      pdf_field->latticeModel().configure(*b, *m_blockforest->get_blocks());
    }
  }
