...
#define BOOST_TEST_MODULE test2d test
...
BOOST_AUTO_TEST_CASE(mirror_position) {
  ...
  auto const  upper_mirror = detail::mirror_position(position, upper_mirror_position);
  const Utils::Vector3d expected_upper_mirror = {position[0], position[1], 2 * upper_mirror_position - position[2]};
  BOOST_TEST(upper_mirror == expected_upper_mirror, boost::test_tools::per_element());
}
