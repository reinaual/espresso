...
pair_force(Particle const &p1, Particle const &p2, Utils::Vector3d const &d,
  ...
  if (coulomb.method == COULOMB_TEST2D_P3M) {
    auto const f1 = coulomb.prefactor *
                    TEST2D_force_charge_contribution(p2.r.p, p1.r.p, q1q2);
    auto const f2 = coulomb.prefactor *
                    TEST2D_force_charge_contribution(p1.r.p, p2.r.p, q1q2);
...
inline double pair_energy(Particle const &p1, Particle const &p2,
  ...
  auto E = [&]() {
    ...
    case COULOMB_TEST2D_P3M:
      return TEST2D_pair_energy(p1, p2) + p3m_pair_energy(q1q2, dist);
#endif
#ifdef SCAFACOS
    case COULOMB_SCAFACOS:
