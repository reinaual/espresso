...
typedef struct {
  ...
  double gap_half;
  /** potential difference between walls */
  double potential_difference;
  /** potential difference between walls per boxl */
  double potential_difference_per_boxl;
} TEST2DParameters;
extern TEST2DParameters test2d_params;

/** @brief set parameters and initialize method */
void TEST2D_set_params(double potential_difference);

/** @brief set coulomb method */
void TEST2D_init();
...
/**
 * @brief assign charges for real and mirror particles to p3m mesh
 * @param particles particles to include in mesh calculation
 */
void TEST2D_charge_assign_P3M(const ParticleRange &particles);

/**
 * @brief calculate energy contribution of @p1 with the image of @p2 and vice versa
...
/**
 * @brief calculate energy contribution of particles with their own mirror charges
...
namespace detail {
    ...
    inline Utils::Vector3d mirror_position(const Utils::Vector3d &position, double height) {
