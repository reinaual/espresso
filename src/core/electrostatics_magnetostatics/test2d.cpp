...
TEST2DParameters test2d_params{0.};
...
void TEST2D_init() {
  ...
  p3m.params.additional_mesh[2] = test2d_params.gap_half;
}
...
void TEST2D_modify_P3M_sums(const ParticleRange &particles) {
  ...
  p3m.square_sum_q = 0;
  // mirror charges have the same q^2 value
...
void TEST2D_assign_image_charge(const Particle &p) {
  if (p.r.p[2] <= test2d_params.gap_half) {
    ...
    auto const pos =
        Utils::Vector3d{p.r.p[0], p.r.p[1], box_geo.length()[2] - p.r.p[2]};
...
double TEST2D_energy_charge_contribution(const Utils::Vector3d &pos1,
  ...
  if (pos1[2] <= test2d_params.gap_half) {
    return p3m_pair_energy(
        -q1q2,
        get_mi_vector(pos2, {pos1[0], pos1[1], -pos1[2]}, box_geo).norm());
  }

  // top image charge
  return p3m_pair_energy(
      -q1q2,
      get_mi_vector(pos2, {pos1[0], pos1[1], box_geo.length()[2] - pos1[2]},
                    box_geo)
          .norm());
}
...
double TEST2D_pair_energy(Particle const &p1, Particle const &p2) {
  ...
  eng += TEST2D_energy_charge_contribution(pos1, pos2, q1q2);
  eng += TEST2D_energy_charge_contribution(pos2, pos1, q1q2);
...
double TEST2D_self_energy(const ParticleRange &particles) {
  auto const eng =
      boost::accumulate(particles, 0.0, [](double eng, auto const &p) {
        return eng +
               TEST2D_energy_charge_contribution(p.r.p, p.r.p, p.p.q * p.p.q);
      });

  return boost::mpi::all_reduce(comm_cart, eng, std::plus<>());
}
...
Utils::Vector3d TEST2D_force_charge_contribution(const Utils::Vector3d &pos1,
  ...
  if (pos1[2] <= test2d_params.gap_half) {
    ...
    auto const d = get_mi_vector(
        pos2, {pos1[0], pos1[1], box_geo.length()[2] - pos1[2]}, box_geo);
...
void TEST2D_self_forces(const ParticleRange &particles) {
  for (auto &p : particles) {
    p.f.f += TEST2D_force_charge_contribution(p.r.p, p.r.p, p.p.q * p.p.q);
  }
}
