...
class EKReaction(ut.TestCase):
    ...
    AGRID = 1.0
    INITIAL_DENSITY = 1.0
    # to test only the reaction part one could set this diffusion coefficient to 0 which would skip all
    # flux-calculations, otherwise
    ...
    def analytic_product_density(self, time: float, reaction_rate: float, initial_density: float) -> float:
        return - 1 / 2 * self.analytic_educt_density(time, reaction_rate, initial_density) + initial_density / 2

    def analytic_educt_density(self, time: float, reaction_rate: float, initial_density: float) -> float:
        return 1 / (1 / initial_density + 2 * reaction_rate * time)

    def analytic_density_base(self, time: float, coeffs, rate_constant: float, init_density: float) -> float:
        """
        ...
        return (init_dens_factor + (order - 1) * factor * time)**(1/(1 - order))
    ...
    def test_reaction(self):
        ...
        for coeff in stoech_coeffs:
            ...
            reactants.append(espressomd.EKSpecies.EKReactant(ekspecies=species, stoech_coeff=-coeff, order=coeff))
        ...
        reactants.append(espressomd.EKSpecies.EKReactant(ekspecies=ek_species_product, stoech_coeff=product_coeff, order=0.0))

        self.system.ekcontainer.solver = eksolver

        reaction = espressomd.EKSpecies.EKBulkReaction(reactants=reactants, coefficient=reaction_rate, lattice=lattice)
        ...
        for i, educt in enumerate(educt_species):
            measured_educt_densities[i] = np.sum(educt[:, :, :].density) / np.product(ek_species_product.shape)
        measured_product_density = np.sum(ek_species_product[:, :, :].density) / np.product(ek_species_product.shape)

        analytic_educt_densities = np.zeros(len(stoech_coeffs))
        for i, coeff in enumerate(stoech_coeffs):
            analytic_educt_densities[i] = coeff * self.analytic_density_base(self.TIME + 0.5, stoech_coeffs, reaction_rate, self.INITIAL_DENSITY)
        ...
        print(analytic_educt_densities, measured_educt_densities)
        print(analytic_product_density, measured_product_density)
        print(analytic_educt_densities/analytic_educt_densities[0], measured_educt_densities/measured_educt_densities[0])
        ...
        np.testing.assert_allclose(measured_educt_densities, analytic_educt_densities, rtol=2 * reaction_rate, atol=0)
        np.testing.assert_allclose(measured_product_density, analytic_product_density, rtol=2 * reaction_rate * len(stoech_coeffs), atol=0)
