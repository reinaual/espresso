    ...
    def test_reaction(self):
        ...
        for i, educt in enumerate(educt_species):
            measured_educt_densities[i] = np.sum(
                educt[:, :, :].density) / np.product(ek_species_product.shape)
        measured_product_density = np.sum(
            ek_species_product[:, :, :].density) / np.product(ek_species_product.shape)
        ...
        print(
            ...
            analytic_educt_densities[0],
            measured_educt_densities /
            measured_educt_densities[0])
